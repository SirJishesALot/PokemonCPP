...
bool Player::incrementSelectPokemonLevel() {
    ...
    if (counter == 0) {
        ...
        if (partyPokemon[x - 1].getPokemonName() == activePokemon.getPokemonName() && partyPokemon[x - 1].getRandomID() == activePokemon.getRandomID()) {
            incrementActivePokemonLevel();
            updateActivePokemon();
        }
        else {
            partyPokemon[x - 1].incrementPokemonLevel();
        }
        return true;
    }
}
